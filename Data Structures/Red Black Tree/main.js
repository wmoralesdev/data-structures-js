...
export default class RedBlackTree extends BinarySearchTree {
    ...
    traverseInOrderWithMeta(node) {
        let traverse = [];

        // Add left node.
        if (node.left) {
            traverse = traverse.concat(this.traverseInOrderWithMeta(node.left));
        }

        // Add root.
        traverse.push(node.value + node.meta.get('color'));

        // Add right node.
        if (node.right) {
            traverse = traverse.concat(this.traverseInOrderWithMeta(node.right));
        }
    ...
    colorShow() {
        let traverse = this.traverseInOrderWithMeta(this.root)
